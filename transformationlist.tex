...
& \;\;\;  insertValue(id(q_i), (col, valueOfColumn(col, p_i)), d')  \nonumber \\
...
& moveProperty(c_s, c_t, p, m, s) = removeProperty(c_s, p, copyProperty(c_s, c_t, p, m, s))
\end{align}

\subsubsection{mergeClasses}
\begin{align}
& mergeClass:  Class \times Class \times Mapping \times Software \rightarrow Software \\
%má jen jednu property, vysvětlit, kde se vzala p
%TODO definovat postupnou aplikaci
& mergeClass(c_1, c_2, m, s) = removeClass(c_2, moveProperty(c_1, c_2, p, m, s)) \\
& \mathbf{where} p \in properties(c_2) \nonumber \\ 
...
\end{align}
\begin{align}
%TODO použití copycolumn tu není dobré - spíš insert
...
%TODO kolize jmen s aplikační úrovní?
%TODO funguje i když je A nebo B prázdné?
\subsubsection{addParent}
\begin{align}
...
%TODO extend interpretation - move children as well
%TODO má jen jednu property
...
%TODO do
\begin{align}
extractParent: Class \times Property \times Inheritance \times Label \times ConsistentSoftware \rightarrow Software \\
extractParent(c, p, i, l, s) = pullUp(c, p, addParent(c, initClass(l), i, m,  addClass(initClass(l) ,s)))
%TODO initClass(l)
%TODO m
\end{align}
...
\begin{align}
insertValue: ID \times Pair \times  Database \rightarrow Database
\end{align}

\textbf{Semantics}:
\begin{align}
insertValue(id, p, d) = d' \implies
\end{align}

\subsubsection{deleteData}
\begin{align}
deleteData: TableSchema \times ID \times  Database \rightarrow Database
\end{align}

\textbf{Semantics}:
\begin{align}
& deleteData(ts, id, d) = d' \implies \exists td \in tableData(d) : id(td) = id \land table(td) = ts \land \nonumber \\
& \;\;\; tableData(d') = tableData(d) \setminus td
\end{align}

%TODO is it needed?
\subsubsection{delete}
\begin{align}
delete: TableSchema \times ID \times Column \times Database \rightarrow Database
\end{align}
